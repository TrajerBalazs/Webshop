...
const products = [
    { id: 1, name: "Fekete Póló", price: 4990, image: "src/cq5dam.web.hebebed.1000.1000.avif", description: "Kényelmes, fekete póló pamut anyagból.", category: "polo" },
    { id: 2, name: "Fehér Cipő", price: 12990, image: "src/fehercipo.webp", description: "Modern fehér cipő mindennapi használatra.", category: "cipo" },
    { id: 3, name: "Kék Nadrág", price: 8990, image: "src/kek.webp", description: "Stílusos kék nadrág, rugalmas anyagból.", category: "nadrag" }
  ];
  
  // Stripe kulcs inicializálása (helyettesítsd a saját publikus kulcsoddal)
  const stripe = Stripe('YOUR_PUBLISHABLE_KEY_HERE'); // Itt használd a saját publikus kulcsodat
  ...
  function openModal(productId) {
    ...
    document.getElementById("modal-product-price").textContent = `${product.price} Ft`;
  ...
  // Fizetési folyamat indítása a Stripe API-val
  async function checkout() {
    try {
      // Létrehozzuk a fizetési session-t a backend szerveren
      ...
      if (!session.id) {
        throw new Error('A session ID nem található');
      }
  
      // Átirányítás a Stripe fizetési oldalra
      ...
      console.error('Hiba történt a fizetési folyamatban:', error);
      alert('Hiba történt a fizetési folyamatban.');
    }
  }
